...
if "users_db" not in st.session_state:
    ...
    st.session_state.users_db = {
        "student1": {
            ...
            "email": "davidleesi2201@gmail.com" # Default demo email
        }
    }
...
def extract_features(username, device_id, password_correct, tracker):
    ...
    # Get current UTC time and add 8 hours
    malaysia_time = dt.datetime.utcnow() + dt.timedelta(hours=8)
    current_hour = malaysia_time.hour
    
    # Now the suspicious hour is ALWAYS 12am-5am in Malaysia
...
if st.session_state.auth_stage == "login":
    ...
    with st.container(border=True):
        ...
        with st.form("login_form"):
            ...
            with col2:
                # The email is now perfectly aligned above the IP address!
                email_address = st.text_input("Email Address", value="@gmail.com", help="The email that will receive the live OTP.").strip()
    ...
    if submit_button:
        if not username or not password or not device_id or not ip_address or not location or not email_address:
            st.warning("⚠️ Please fill in all signal fields to simulate the login.")
        else:
            # Dynamically map the typed email to the user for this session
            if username in st.session_state.users_db:
                st.session_state.users_db[username]["email"] = email_address
            ...
            if current_time < tracker["blocked_until"]:
                ...
                if not password_correct:
                    ...
                    if risk_level == "HIGH":
                        ...
                        st.warning("⚠️ Unfamiliar device context detected. Routing to CAPTCHA...")
                        time.sleep(2) 
                        num1, num2 = random.randint(1, 10), random.randint(1, 10)
                        st.session_state.captcha_answer = num1 + num2
                        st.session_state.captcha_question = f"What is {num1} + {num2}?"
